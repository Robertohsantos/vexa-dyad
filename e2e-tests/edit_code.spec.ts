...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-vexa.tsx");
  ...
  await po.page.getByText("made-with-vexa.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithVexa = ;");
  ...
  expect(editedFile).toContain("export const MadeWithVexa = ;");
});
