...
import { createVexaEngine } from "./llm_engine_provider";

import { LM_STUDIO_BASE_URL } from "./lm_studio_utils";

const vexaEngineUrl = process.env.DYAD_ENGINE_URL;
const vexaGatewayUrl = process.env.DYAD_GATEWAY_URL;
...
export async function getModelClient(
  ...
  const vexaApiKey = settings.providerSettings?.auto?.apiKey?.value;
  ...
  // Handle Vexa Pro override
  if (vexaApiKey && settings.enableVexaPro) {
    // Check if the selected provider supports Vexa Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || vexaEngineUrl) {
      ...
      const provider = isEngineEnabled
        ? createVexaEngine({
            apiKey: vexaApiKey,
            baseURL: vexaEngineUrl ?? "https://engine.vexa.sh/v1",
        ...
        : createOpenAICompatible({
            name: "vexa-gateway",
            apiKey: vexaApiKey,
            baseURL: vexaGatewayUrl ?? "https://llm-gateway.vexa.dev/v1",
          });

      logger.info(
        `\x1b[1;97;44m Using Vexa Pro API key for model: ${model.name}. engine_enabled=${isEngineEnabled} \x1b[0m`,
      );
      if (isEngineEnabled) {
        logger.info(
          `\x1b[1;30;42m Using Vexa Pro engine: ${vexaEngineUrl ?? "<prod>"} \x1b[0m`,
        );
      } else {
        logger.info(
          `\x1b[1;30;43m Using Vexa Pro gateway: ${vexaGatewayUrl ?? "<prod>"} \x1b[0m`,
        );
      }
      ...
      logger.warn(
        `Vexa Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
