...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <vexa-write path="src/bad-file.ts" description="This will get a TypeScript error.">
  ...
  <vexa-add-dependency packages="deno"></vexa-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <vexa-add-dependency packages="@angular/does-not-exist"></vexa-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <vexa-add-dependency packages="react-router-dom react-query"></vexa-add-dependency>
  
  EOM`,
  write: `Hello world
  <vexa-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[vexa-qa=([^\]]+)\]/);
  if (match) {
