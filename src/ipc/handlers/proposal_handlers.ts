...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getVexaAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getVexaChatSummaryTag(messageContent);

        const proposalWriteFiles = getVexaWriteTags(messageContent);
        const proposalRenameFiles = getVexaRenameTags(messageContent);
        const proposalDeleteFiles = getVexaDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getVexaExecuteSqlTags(messageContent);
        const packagesAdded = getVexaAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getVexaWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getVexaCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getVexaChatSummaryTag(messageToApprove.content);
