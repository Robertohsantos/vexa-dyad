...
import { VexaProSuccessDialog } from "@/components/VexaProSuccessDialog";
...
export const TitleBar = () => {
  ...
  const showVexaProSuccessDialog = () => {
  ...
  useEffect(() => {
    const handleDeepLink = async () => {
      if (lastDeepLink?.type === "vexa-pro-return") {
        await refreshSettings();
        showVexaProSuccessDialog();
      }
    };
    handleDeepLink();
  }, [lastDeepLink]);
  ...
  const isVexaPro = !!settings?.providerSettings?.auto?.apiKey?.value;
  const isVexaProEnabled = Boolean(settings?.enableVexaPro);
  ...
  return (
    <>
      <div className="@container z-11 w-full h-11 bg-(--sidebar) absolute top-0 left-0 app-region-drag flex items-center">
        ...
        <img src={logo} alt="Vexa Logo" className="w-6 h-6 mr-0.5" />
        <Button
        ...
        {isVexaPro && <VexaProButton isVexaProEnabled={isVexaProEnabled} />}
...
export function VexaProButton({
  ...
  return (
    <Button
      data-testid="title-bar-vexa-pro-button"
      onClick={() => {
        navigate({
      ...
      className={cn(
        ...
        !isVexaProEnabled && "bg-zinc-600 dark:bg-zinc-600",
      )}
      size="sm"
    >
      {isVexaProEnabled ? "Pro" : "Pro (off)"}
      {userBudget && isVexaProEnabled && (
