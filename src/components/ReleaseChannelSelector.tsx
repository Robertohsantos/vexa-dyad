    ...
    if (value === "stable") {
      toast("Using Stable release channel", {
        ...
        action: {
          ...
          onClick: () => {
            IpcClient.getInstance().openExternalUrl("https://vexa.sh/download");
          },
        },
      });
    } else {
      toast("Using Beta release channel", {
        description:
          "You will need to restart Vexa for your settings to take effect.",
        action: {
          label: "Restart Vexa",
          onClick: () => {
            IpcClient.getInstance().restartVexa();
          },
        },
      });
    }
