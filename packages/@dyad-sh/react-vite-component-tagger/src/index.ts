 ...
 */
export default function vexaTagger(): Plugin {
  return {
    name: "vite-plugin-vexa-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-vexa-id ───────────────
              ...
              console.warn(
                `[vexa-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[vexa-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
