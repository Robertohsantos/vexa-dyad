  ...
  try {
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "vexa-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `vexa-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `vexa-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `vexa-${version}.Setup.exe`,
      `vexa-darwin-arm64-${version}.zip`,
      `vexa-darwin-x64-${version}.zip`,
      `vexa_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      "RELEASES",
    ];
